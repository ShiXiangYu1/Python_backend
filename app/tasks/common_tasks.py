...
class SQLAlchemyTask(Task):
    ...
    def update_progress(self, db_task_id: str, progress: int, result: Any = None) -> bool:
        """
        更新任务进度
    ...
    def batch_process(self, items: List[Any], process_func: Callable, batch_size: int = 100, task_id: Optional[str] = None) -> Dict[str, Any]:
        """
        批量处理
        ...
        # 初始化进度
        if task_id:
            self.update_progress(task_id, 0, {"total_items": total_items, "batch_size": batch_size})
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                # 获取任务对象
                task = wrapper.task if hasattr(wrapper, 'task') else celery_app.current_task
                ...
                logger.info(f"任务将在 {retry_delay:.2f} 秒后重试 (尝试 {retries + 1}/{max_retries})")
...
def long_running_task(self, seconds: int = 10, task_id: Optional[str] = None) -> Dict[str, Any]:
    """
    长时间运行的示例任务
...
def batch_process_items(self, items: List[Any], task_id: Optional[str] = None) -> Dict[str, Any]:
    """
    批量处理数据项
...
def cleanup_old_data(self, days: int = 30, task_id: Optional[str] = None) -> Dict[str, Any]:
    """
    清理旧数据
