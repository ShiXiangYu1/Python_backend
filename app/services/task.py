...
class TaskService(BaseService):
    ...
    async def update_task_status(
        ...
        if status in (TaskStatus.SUCCEEDED, TaskStatus.FAILED, TaskStatus.REVOKED) and not task.completed_at:
    ...
    async def sync_task_status_from_celery(self, db: AsyncSession, task_id: uuid.UUID) -> Optional[Task]:
        """
        从Celery同步任务状态
    ...
    async def get_task_count(
        ...
        if status:
            return {
                "total": total,
                status.lower() if isinstance(status, str) else status.value.lower(): total
            }
    ...
    def _get_cached_task(self, task_id: uuid.UUID) -> Optional[Task]:
        ...
        try:
            ...
            task.status = TaskStatus(task_data["status"]) if task_data["status"] else None
            task.priority = int(task_data["priority"]) if task_data["priority"] else None
            ...
            task.user_id = uuid.UUID(task_data["user_id"]) if task_data["user_id"] else None
            task.model_id = uuid.UUID(task_data["model_id"]) if task_data["model_id"] else None
