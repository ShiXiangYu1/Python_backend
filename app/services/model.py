...
class ModelService(CRUDBase[Model, ModelCreate, ModelUpdate]):
    ...
    async def get_multi_by_owner(
        ...
        返回:
            List[Model]: 模型列表
        """
        query = select(Model).where(Model.owner_id == owner_id).offset(skip).limit(limit)
    ...
    async def get_models_with_pagination(
        self, db: AsyncSession, *, owner_id: Optional[str] = None, 
        skip: int = 0, limit: int = 100, public_only: bool = False
    ...
    async def upload_model_file(
        ...
        file_path = os.path.join(
            ...
            f"{model_id}_{os.path.basename(file.filename)}"
        )
    ...
    async def _save_file_with_hash(self, file: UploadFile, file_path: str) -> Tuple[str, int]:
        """
        保存文件并计算哈希值
    ...
    async def deploy_model(
        ...
        # 检查模型是否可以部署
        if model.status not in [ModelStatus.UPLOADED, ModelStatus.VALID, ModelStatus.UNDEPLOYED]:
            return None
...
class ModelVersionService(CRUDBase[ModelVersion, ModelVersionCreate, ModelVersionUpdate]):
    """
    模型版本服务类
    ...
    async def set_current_version(
        ...
        """
        # 取消所有版本的当前状态
        query = select(ModelVersion).where(ModelVersion.parent_model_id == parent_model_id)
        result = await db.execute(query)
