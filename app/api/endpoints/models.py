...
from app.schemas.model import (
    Model, ModelCreate, ModelUpdate, ModelDeploy,
    ModelVersion, ModelVersionCreate, ModelVersionUpdate
)
...
async def upload_model_file(
    ...
    # 上传模型文件
    updated_model = await model_service.upload_model_file(db, model_id=model_id, file=file)
...
async def deploy_model(
    ...
    # 部署模型
    updated_model = await model_service.deploy_model(db, model_id=model_id, config=config)
    if updated_model is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="部署模型失败"
...
async def read_model_versions(
    ...
    # 获取版本列表
    versions = await model_version_service.get_multi_by_model(db, parent_model_id=model_id)
    return versions
